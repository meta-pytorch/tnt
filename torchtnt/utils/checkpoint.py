...
class CheckpointManager:
    ...
    def __init__(
        ...
        self._failed_checkpoint_removals: SimpleQueue[CheckpointPath] = SimpleQueue()
    ...
    def remove_checkpoint(self) -> None:
        ...
        if self._pg_wrapper.get_rank() == 0:
            failed_checkpoint_path = self._take_failed_checkpoint_removal()
            if failed_checkpoint_path is not None:
                self._remove_checkpoint_from_filesystem(failed_checkpoint_path)
            self._remove_checkpoint_from_filesystem(worst_ckpt_path)

    def _remove_checkpoint_from_filesystem(
        self, checkpoint_path: CheckpointPath
    ) -> None:
        ...
        if callable(rm_in_background):
            try:
                ...
                logger.error(
                    f"Failed to schedule checkpoint '{checkpoint_path}' for background removal. "
                    f"No removal was attempted. Exception: {exc}"
                )
                self._queue_failed_checkpoint_removal(checkpoint_path)
            else:
                future.add_done_callback(
                    lambda completed_future: self._handle_checkpoint_removal_completion(
                        checkpoint_path,
                        completed_future,
                    )
                )
            ...
            try:
                self._file_system.rm(checkpoint_path.path, recursive=True)
            except Exception as exc:
                logger.error(
    ...
    def _queue_failed_checkpoint_removal(self, checkpoint_path: CheckpointPath) -> None:
        self._failed_checkpoint_removals.put(checkpoint_path)

    def _take_failed_checkpoint_removal(self) -> Optional[CheckpointPath]:
        try:
            return self._failed_checkpoint_removals.get_nowait()
        except Empty:
            return None
